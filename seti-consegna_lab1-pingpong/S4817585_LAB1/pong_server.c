...
void sigchld_handler(int signum)
{
	...
	if (waitpid(-1, &status, WNOHANG) == -1)
		fail_errno("Pong Server cannot wait for children in SIGCHLD handler");
}
...
/*** TO BE DONE START ***/
		...
		if ( clock_gettime(CLOCK_TYPE, &time2) < 0)
...
/*** TO BE DONE START ***/
		...
		*pong_port = port_number;//salvo porta effimera
		
		if ((gai_rv = getaddrinfo("localhost",port_number_as_str,&gai_hints,&pong_addrinfo)) != 0)
			fail_errno(strerror(errno));
	
		if((bind_rv = bind(udp_socket, pong_addrinfo->ai_addr, pong_addrinfo->ai_addrlen)) == -1)
			fail_errno(strerror(errno));
...
/*** TO BE DONE START ***/
	...
	if((server_socket = socket(gai_hints.ai_family, gai_hints.ai_socktype, gai_hints.ai_protocol)) == -1){
		fail_errno(strerror(errno));
	}
