...
/*** TO BE DONE START ***/
		...
		if(recv_bytes < 0){
			debug("Error, retrying recieving msg\n");
			recv_errno = errno;
		}
		else recv_errno = 0;	
		/*
		recv_bytes = read(ping_socket, answer_buffer, sent_bytes);
		
		if(recv_bytes <= 0){
			sleep(timeout/3000);
			if(recv_bytes < 0){
				printf("Error receiving data, repeat");
				continue;
			}
		}
		*/
		...
		timeouts.tv_sec = timeout/1000;  // Set the seconds part of the timeout
		timeouts.tv_usec = 0;
		FD_ZERO(&read_fds);//inizializzo 
		FD_SET(ping_socket, &read_fds);//setto

		// Use the select function to wait for data or a timeout
		int ready = select(ping_socket + 1, &read_fds, NULL, NULL, &timeouts);
		if(ready < 1){
			if (ready == -1) {
				fail_errno(strerror(errno));
			}
		...
		} else if (FD_ISSET(ping_socket, &read_fds))// Data is available on the socket
				recv_bytes = read(ping_socket, answer_buffer, sizeof(answer_buffer));*/
/*** TO BE DONE END ***/
...
/*** TO BE DONE END ***/
	...
	if ((ping_socket = socket(gai_hints.ai_family, gai_hints.ai_socktype /*|| SOCK_NONBLOCK*/, gai_hints.ai_protocol)) == -1)
...
/*** TO BE DONE START ***/
	...
	if((gai_rv = getaddrinfo(argv[1],argv[2],&gai_hints,&server_addrinfo)) != 0)
		fail_errno("Could not get the server info");
/*** TO BE DONE END ***/
