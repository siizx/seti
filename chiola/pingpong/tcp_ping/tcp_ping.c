...
double do_ping(size_t msg_size, int msg_no, char message[msg_size], int tcp_socket)
{
	...
	{
		int err = errno;
		perror("Error writing the message size into the buffer.");
		return err;
	}
	...
	{
		int err = errno;
		perror("Error while getting and storing time in send_time.");
		return err;
	}
	...
	{
		int err = errno;
		perror("Error while using blocking_write_all, sending the message through the socket.");
		return errno;
	};
	/*** TO BE DONE END ***/
	...
	{
		int err = errno;
		perror("Error while storing current time in recv_time.");
		return err;
	}
