...
/*** TO BE DONE START ***/
		if (clock_gettime(CLOCK_TYPE, &send_time))
			fail_errno("Cannot get send-time");
...
/*** TO BE DONE START ***/
		recv_bytes = recv(ping_socket, answer_buffer, sent_bytes/*sizeof(answer_buffer)*/, 0);
		if (recv_bytes < 0)
			recv_errno = errno;
...
/*** TO BE DONE START ***/
	gai_hints.ai_family = AF_INET;    // Permette IPv4
	gai_hints.ai_socktype = SOCK_DGRAM; // UDP
	//gai_hints.ai_protocol = IPPROTO_UDP; // protocollo UDP, se si omette si usa il protocollo di default definito da ai_socktype
...
/*** TO BE DONE START ***/
	if (fcntl(ping_socket, F_SETFL, O_NONBLOCK) == -1)
		fail_errno("UDP Ping could not set socket flags");
...
/*** TO BE DONE START ***/
	if (connect(ping_socket, pong_addrinfo->ai_addr, pong_addrinfo->ai_addrlen) == -1)
		fail_errno("UDP Ping could not connect to Pong server");
...
/*** TO BE DONE START ***/
	...
	//gai_hints.ai_protocol = IPPROTO_TCP; // protocollo TCP, se si omette si usa il protocollo di default definito da ai_socktype
...
/*** TO BE DONE START ***/
	...
	// Uso la funzione socket() per creare un socket
	ask_socket = socket(server_addrinfo->ai_family, server_addrinfo->ai_socktype, server_addrinfo->ai_protocol);
	// Controllo se la creazione del socket è andata a buon fine
	if (ask_socket == -1)
		fail_errno("UDP Ping could not create socket");
	// Uso la funzione connect() per connettermi al server
...
/*** TO BE DONE START ***/
	if (strncmp(answer, "OK", 2) != 0)
		fail("UDP Ping received wrong answer from Pong server");
