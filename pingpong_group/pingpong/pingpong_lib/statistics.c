...
/*** TO BE DONE START ***/
	double delta = (double)(last->tv_sec - previous->tv_sec) * 1000.0;
	delta += (double)(last->tv_nsec - previous->tv_nsec) / 1000000.0;
	return delta;
		      ...
		      double rtt[repeats], int msg_sz, double resolution)
{
	...
	for (j = 0, ns = h_min; j < N_HISTOGRAM_ITEMS; j++, ns += h_incr)
		fprintf(outf, "%lg %d\n", ns, histogram[j]);
	fprintf(outf, "\n");
	if (rtt[median] > 0.0)
