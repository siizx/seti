...
/*** TO BE DONE START ***/
	...
	if (sprintf(message, "%d", msg_no) < 0)
		fail_errno("Error writing message number to buffer");
...
/*** TO BE DONE START ***/
	...
	if (clock_gettime(CLOCK_TYPE, &send_time) == -1)
		fail_errno("Error getting sent_time");
...
/*** TO BE DONE START ***/
	...
	if (sent_bytes != (ssize_t)msg_size)
...
/*** TO BE DONE START ***/
	if (clock_gettime(CLOCK_TYPE, &recv_time) == -1)
		fail_errno("Error getting recv_time");
...
/*** TO BE DONE START ***/
	...
	if (tcp_socket == -1)
		fail_errno("TCP Ping could not create socket");
	...
	if (connect(tcp_socket, server_addrinfo->ai_addr, server_addrinfo->ai_addrlen) == -1)
		fail_errno("TCP Ping could not connect to Pong server");
...
/*** TO BE DONE START ***/
	if (strcmp(answer, "OK\n") != 0)
		fail("TCP Ping received a negative answer from Pong server");
