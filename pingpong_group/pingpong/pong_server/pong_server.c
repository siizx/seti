...
/*** TO BE DONE START ***/
		...
		if (gai_rv != 0)
			fail_errno("UDP Pong could not get address info");
...
/*** TO BE DONE START ***/
	...
	if (server_socket == -1)
		fail_errno("Pong Server cannot create socket");
	...
	if (listen(server_socket, LISTENBACKLOG) == -1)
		fail_errno("Pong Server cannot listen on socket");
